$(document).ready(function () {
        ...
        $.getJSON('/api/stocksvalue/' 
               ...
               + "&endDate=" + endDate
                 , function (chartData) {
                     ...
                     // GET STOCK TICKER STRING - This function will extract the current stock from the data-set.  We'll use this to create unique element IDs for the SVG and axises.
                     ...
                     var minYRange = d3.min(chartData, function (d) { return d.ValueLow; }),
                         maxYRange = d3.max(chartData, function (d) { return d.ValueHigh; });
                     ...
                     var y = d3.scale.linear()
                         .range([height, 0])
                         .domain([0, d3.max(chartData, function (d) { return d.ValueHigh; })]);
                     ...
                     var heatcolor = ["#f8696b", "#f98570", "#fa9d75", "#fdd17f", "#ffeb84", "#e0e383", "#c1da81", "#a2d07f", "#83c77d", "#63be7b"]; // GREEN > YELLOW > RED
                    // var heatcolor = [ "#e6edf6","#d7e3f1", "#c8d8eb", "#b9cde5", "#aac2e0", "#9bb7da", "#8cadd4", "#7da2cf", "#6e97c9", "#5f8cc3", "#4f81bd"];  // LIGHT BLUE > DARK BLUE

                     //FUNCTION FOR APPLYING HEAT BASED ON RANGE OF DATA VALUE
                     var heat = d3.scale.quantile()
                         .range(heatcolor)
                         .domain([maxYRange, minYRange]);

                     // ADD BARS 
                     svg.selectAll(".bar")
                         .data(chartData)
                       .enter().append("rect")
                         .style("fill", function (d) { return heat(d.ValueClose); })
    ...
    var table = d3.select("#div-stocktable")
      ...
      //  .attr("style", 'width: 0px ; align: "left"')
    ;
